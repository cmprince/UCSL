...
**Removing outliers** had little effect on the quartile calculations: the median shifted `tdur.median - tdur.z3.median = ` `r tdur.median-tdur.z3.median` sec, and the IQR was reduced by only `tdur.iqr - tdur.z3.iqr = ` `r tdur.iqr-tdur.z3.iqr` sec. However, the mean shifted by a significant amount, `tdur.mean - tdur.z3.mean = ` `r tdur.mean-tdur.z3.mean` sec. This is due to removing a few very large data points skewing mean. This is observed by noticing that the overall range reduced from `r tdur.range` sec to `r tdur.z3.range` sec.
...
hist(cbdata.z3$tripduration, main = "Trip Duration, z_sc<3", xlab = "time (sec)", col='red')
hist(log(cbdata.z3$tripduration), main = "log(Trip Duration, z_sx<3)", xlab = "log(time (sec))", col='red')
